...
describe 'dnsmasq::dns' do
  ...
  describe 'dnsmasq dns.conf template' do
    it 'creates the /etc/dnsmasq.d/dns.conf template' do
      expect(chef_run_converge).to create_template(dns_conf).
        with(owner: 'dnsmasq')
    end

    it 'notifies service[dnsmasq] to restart' do
      expect(chef_run_converge.template(dns_conf)).to notify('service[dnsmasq]').
        to(:restart)
    end
  end
  ...
  describe 'dnsmasq resolv_dnsmasq.conf template' do
    ...
    it 'creates the /etc/resolv_dnsmasq.conf template' do
      expect(chef_run_converge).to create_template(resolv_conf).
        with(owner: 'dnsmasq')
    end

    it 'notifies service[dnsmasq] to restart' do
      expect(chef_run_converge.template(resolv_conf)).to notify('service[dnsmasq]').
        to(:restart)
    end
  end
  ...
  describe 'dnsmasq hosts_dnsmasq template' do
    ...
    it 'creates the /etc/hosts_dnsmasq template' do
      expect(chef_run_converge).to create_template(hosts_conf).
        with(owner: 'dnsmasq')
    end

    it 'notifies service[dnsmasq] to restart' do
      expect(chef_run_converge.template(hosts_conf)).to notify('service[dnsmasq]').
        to(:restart)
    end
  end
end
